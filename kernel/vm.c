...
hy_kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  ...
  for (i = PGROUNDUP(start); i < start + sz; i+=PGSIZE)
  {
    ...
    if((*pte & PTE_V) == 0){
      panic("kvmcopymappings: pte is invalid");
    }
    pa = PTE2PA(*pte);
...
uint64
hy_kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz){
  if(newsz >= oldsz){
    return oldsz;
  }
...
kvminit()
{
  ...
  kernel_pagetable = hy_kvminit_newpgtbl();
  // 全局内核页表仍需要映射 CLINT
...
hy_kvminit_newpgtbl()
{
  ...
  hy_kvm_map_pagetable(pgtbl);
  return pgtbl;
}

void
hy_kvm_map_pagetable(pagetable_t pgtbl)
{
...
hy_kvm_free_kernelpgtbl(pagetable_t pagetable){
  for (int i = 0; i < 512; i++)
  {
    ...
    if((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0){  // 如果该页表项指向更低一级的页表
      hy_kvm_free_kernelpgtbl((pagetable_t)child);  // 递归释放低一级页表及其页表项
      pte = 0;
    }
  }
...
//递归打印页表
int hy_pgtblprint(pagetable_t pagetable, int depth){
  // there are 2^9 =  512 PTEs in a page table.
  for (int i = 0; i < 512; i++)
  {
    pte_t pte = pagetable[i];
    if(pte & PTE_V){  // 页表项有效
      ...
      //如果该节点不是叶子结点，递归打印叶子结点
      //当前页表条目没有设置任何访问权限（读、写或执行），为中间页表
      if((pte & (PTE_R | PTE_W | PTE_X)) == 0){
        // this PTE points to a lower-level page table.
        uint64 child = PTE2PA(pte);
        hy_pgtblprint((pagetable_t)child, depth + 1);
      }
    }
  }
  return 0;
...
//打印页表
int hy_vmprint(pagetable_t pagetable){
  printf("page table %p\n", pagetable);
  return(hy_pgtblprint(pagetable, 0));
}
